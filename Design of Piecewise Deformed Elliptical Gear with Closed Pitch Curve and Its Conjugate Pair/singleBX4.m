...
m12=1.4;            %0.5;可不为整数
m13=2.5;
m14=1/(4-1/m11-1/m12-1/m13);            %0.5;可不为整数
...
A1=mt*pi*z1/((m11*temM11*quadl(fun1,0,pi/m11))+(m12*temM12*quadl(fun2,pi/m11,pi/m11+pi/m12))+(m13*temM13*quadl(fun3,pi/m11+pi/m12,pi/m11+pi/m12+pi/m13))+(m14*temM14*quadl(fun4,pi/m11+pi/m12+pi/m13,2*pi)));% %数值积分法计算主动轮半长轴
...
elseif sita1(i)>2*pi/(DN1*m11)+2*pi/(DN1*m12)+2*pi/(DN1*m13) && sita1(i)<=2*pi+0.000002
  r1(i)=p1/(1-pxl1*cos(m14*(sita1(i)-2*pi/(DN1*m11)-2*pi/(DN1*m12)-2*pi/(DN1*m13))+3*2*pi/DN1));%极径
    r1BX4(i)=r1(i);%极径
     FD4=i;     
end
end
...
plot(r1BX2x(FD1:FD2),r1BX2y(FD1:FD2),'Color','b','LineStyle','-','linewidth',2);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%画直角坐标椭圆
...
plot(r1BX4x(FD3:FD4),r1BX4y(FD3:FD4),'Color','k','LineStyle',':','linewidth',2);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%画直角坐标椭圆


rBXx=[r1BX1x,r1BX2x(FD1:FD2),r1BX3x(FD2:FD3),r1BX4x(FD3:FD4)];%%%%%输出到cad
rBXy=[r1BX1y,r1BX2y(FD1:FD2),r1BX3y(FD2:FD3),r1BX4y(FD3:FD4)];%%%%%输出到cad
% mattoacad('mattoacad',rBXx,rBXy); %%%%%输出到cad

% line([min(rx)-20,max(rx)+20],[0,0],'Color','r','LineStyle','-.','linewidth',1);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%画直角坐标椭圆
% line([0,0],[min(ry)-20,max(ry)+20],'Color','r','LineStyle','-.','linewidth',1);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%画直角坐标椭圆
